...
void loop() {
  ...
  if (Timer_Motor.isReady()) {
    ...
    if (cfg.status == 0) analogWrite(motor_EN, 0);
    if (cfg.status == 0) digitalWrite(motor_EN, LOW);  // Устанавливаем логический 0 на входах драйвера L_EN и R_EN, значит выходы M+ и M- перейдут в состояние высокого импеданса и мотор будет электрически отключён.
  }
